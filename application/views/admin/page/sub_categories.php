          ...
          <div class="col-lg-3 col-sm-6 col-xs-12 item_holder">
            <div class="block_wallpaper add_wall_category" style="box-shadow:0px 3px 8px rgba(0, 0, 0, 0.3)">  <div class="wall_category_block" style="z-index: 1">
                ...
                <?php if($this->db->get_where('tbl_verify', array('id' => '1'))->row()->android_envato_purchased_status==1)
                  {
                ?>
                  <a href="javascript:void(0)" data-type="sub_category" data-sub_id="<?php echo $row->id;?>" data-title="<?php echo $row->sub_category_name;?>" data-id="<?php echo $row->category_id;?>" data-image="<?=($row->sub_category_image!='') ? IMG_PATH.$row->sub_category_image : ''?>" class="btn_notification <?=($row->status) ? 'show_notification_icon' : 'hide_notification_icon'?>" data-toggle="tooltip" data-tooltip="<?=$this->lang->line('send_notification_lbl')?>" style="width: 30px;height: 30px;z-index: 1;"><div><i class="fa fa-bell"></i></div></a>
                ...
                <ul>                
                  ...
                  <li>
                    <div class="row toggle_btn">
                      <input type="checkbox" id="enable_disable_check_<?=$i?>" data-id="<?=$row->id?>" data-table="tbl_sub_category" data-column="status" class="cbx hidden enable_disable" <?php if($row->status==1){ echo 'checked';} ?>>
